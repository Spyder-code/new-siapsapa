...
@section('breadcrumb')
    <div class="row">
        <x-breadcrumb_left :links="[
            ['name' => 'Dashboard', 'url' => '/'],
            ['name' => 'Anggota', 'url' => '#'],
            ['name' => 'Detail', 'url' => '#'],
        ]" :title="'Detail Anggota'" :description="'Anggota'" />
    </div>
@endsection
@section('content')
                    ...
                    <div class="d-flex gap-4">
                        ...
                        @if ($anggota->is_cetak)
                            <form action="{{ route('percetakan.print') }}" method="post" target="_blank">
                                @csrf
                                <button type="submit" name="transaction_id" class="btn btn-sm btn-success"
                                    value="{{ $anggota->cetak->id }}"><i class="fas fa-print"></i> Print</button>
                            </form>
                        @endif
                    </div>
                </div>
